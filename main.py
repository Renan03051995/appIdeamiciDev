...
class MainApp(App):
    ...
    def on_start(self):
        ...
        pagina_adicionarvendas = self.root.ids["adicionardocpage"]
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-155c4-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            ...
            homepage.ids["label_total_vendas"].text = f"[color=#A9A9A9]Seu ID único: [b]{self.id_vendedor}[/b][/color]"
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-155c4-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        for item in list(lista_clientes.children):
            ...
            # foto -> rg-cpf-cnh.png / Label -> rg-cpf-cnh -> rg-cpf-cnh-> rg-cpf-cnh.png
            try:
                texto = item.text
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade)==float):
            ...
            requests.post(f"https://aplicativovendashash-155c4-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                          data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-155c4-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
